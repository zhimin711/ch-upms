...
@RestController
@RequestMapping("/dict")
@Tag(name = "数据字典-管理端", description = "数据字典管理相关接口")
public class DictAdminController {
    @Autowired
    private IDictService dictService;
    
    
    @Operation(summary = "分页查询", description = "分页查询数据字典")
    ...
    @Operation(summary = "修改数据字典", description = "根据ID修改数据字典")
    @PutMapping({"/{id:[0-9]+}"})
    public Result<Boolean> edit(@PathVariable Long id, @RequestBody Dict record) {
        ...
        return ResultUtils.wrapFail(() -> {
            ...
            if (orig != null && !CommonUtils.isEquals(id, orig.getId())) {
                ExUtils.throwError(PubError.EXISTS, "代码");
            }
    ...
    @Operation(summary = "查询数据字典详情", description = "根据ID查询数据字典详情")
    @GetMapping({"/{id:[0-9]+}"})
